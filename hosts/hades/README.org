...
sudo mkfs.ext4 -L nixos /dev/nvme1n1p1
sudo mkfs.fat -F 32 -n boot /dev/nvme1n1p2
...
HOST=hades
USER=brian
sudo mv dotfiles/ /etc/dotfiles
cd /etc/dotfiles/
sudo USER=$USER nixos-install --root /mnt --impure --flake .#$HOST
...
* Overview
My main driver for work on Linux.

Have a separate Windows rig for gaming/adobe (Midori).
